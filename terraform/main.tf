  ...
  container_definitions = jsonencode([
    {
      ...
      environment = [
        {
          name  = "CLIENT_ID"
          value = "frontend-itau" # Substitua por referência ao Secrets Manager
        },
        {
          name  = "CLIENT_SECRET"
          value = "segredo123" # Substitua por referência ao Secrets Manager
        },
        {
          name  = "JWT_SECRET"
          value = "itau-secret-itau-secret-itau-secret" # Substitua por referência ao Secrets Manager
        }
      ],
      logConfiguration = {
        logDriver = "awslogs",
...
resource "aws_security_group" "lb_sg" {
  ...
  # Não é necessário um ingress para 8080 no SG do LB, pois ele escuta na porta 80 e encaminha para 8080.
  # O 8080 deve ser aberto no SG do Fargate para o tráfego vindo do LB.
